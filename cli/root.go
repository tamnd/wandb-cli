// Package cli assembles the wandb command tree from the wandb
...
// NewApp assembles the kit application from the wandb domain. The
...
//
// To add a command, declare it in wandb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := wandb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(wandb.Domain{}).Register(app)
