...
describes what users get. Pick a format with `-o`, or let wandb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wandb <command> -o table   # aligned columns for reading
wandb <command> -o jsonl   # one JSON object per line, for piping
wandb <command> -o json    # a single JSON array
wandb <command> -o csv     # spreadsheet friendly
wandb <command> -o tsv     # tab-separated
wandb <command> -o url     # just the URL column
wandb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wandb <command> --fields id,title,url
```
...
```bash
wandb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wandb <command>            # a table, because this is a terminal
wandb <command> | wc -l    # JSONL, because this is a pipe
```
